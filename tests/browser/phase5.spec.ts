...
test('renders isolated multi-camera passes and releases their resources', async ({
  ...
  await page.goto('/phase5.html');

  const status = page.getByTestId('status');
  const metrics = page.getByTestId('phase5-metrics');
  const canvas = page.getByTestId('phase5-canvas-host').locator('canvas');
  await expect(status).toHaveText('C5 multi-camera scene ready');
  await page.evaluate(() => window.__FLIXEL_PIXI_PHASE5__?.seekAnimation?.(0));
  ...
  await expect(page).toHaveScreenshot('phase5-cameras.png', {
    clip: {
  ...
  await expect
    .poll(() => page.evaluate(() => window.__FLIXEL_PIXI_PHASE5__?.destroyed))
    .toBe(true);
});
...
test('keeps pointer conversion and render targets correct after resize', async ({
  ...
  await expect(page.getByTestId('status')).toHaveText(
    'C5 multi-camera scene ready',
  );
  await page.evaluate(() => window.__FLIXEL_PIXI_PHASE5__?.seekAnimation?.(0));

  const worldPoint = await page.evaluate(() => {
    return window.__FLIXEL_PIXI_PHASE5__?.pointerToWorld?.(
      'overview',
      420 + 180,
      20 + 120,
    );
  ...
  const resized = await page.evaluate(() => {
    return window.__FLIXEL_PIXI_PHASE5__?.resize?.(820, 300, 2);
  });
...
test('animates the follow target and camera on deterministic fixed steps', async ({
  ...
  const initial = await page.evaluate(() =>
    window.__FLIXEL_PIXI_PHASE5__?.seekAnimation?.(0),
  );
  const advanced = await page.evaluate(() =>
    window.__FLIXEL_PIXI_PHASE5__?.advanceAnimation?.(120),
  );
  ...
  await page.evaluate(() => window.__FLIXEL_PIXI_PHASE5__?.resumeAnimation?.());
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          window.__FLIXEL_PIXI_PHASE5__?.animationState?.().simulationSeconds ??
          0,
      ),
    )
    .toBeGreaterThan((advanced?.simulationSeconds ?? 0) + 0.05);
  await page.evaluate(() => window.__FLIXEL_PIXI_PHASE5__?.pauseAnimation?.());
});
