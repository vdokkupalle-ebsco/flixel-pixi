...
test('renders one world through two isolated cameras and tears it down', async ({
  ...
  await page.goto('/rendering.html');

  const status = page.getByTestId('status');
  const host = page.getByTestId('rendering-canvas-host');
  ...
  await expect
    .poll(() =>
      page.evaluate(() => window.__FLIXEL_PIXI_RENDERING__?.destroyed),
    )
    .toBe(true);
});
...
test('resizes camera targets across high-DPI and narrow layouts', async ({
  ...
  await expect(page.getByTestId('status')).toContainText(
    'Rendering demo ready',
  );

  const highDpi = await page.evaluate(() =>
    window.__FLIXEL_PIXI_RENDERING__?.resize?.(720, 650, 2),
  );
  ...
  const restored = await page.evaluate(() =>
    window.__FLIXEL_PIXI_RENDERING__?.resize?.(900, 380, 1),
  );
  ...
  await expect(
    page.getByTestId('rendering-canvas-host').locator('canvas'),
  ).toHaveCount(1);
  expect(pageErrors).toEqual([]);
});
