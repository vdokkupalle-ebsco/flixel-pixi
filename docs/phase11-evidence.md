...
- **Real-browser Playwright tests** (`tests/browser/phase11.spec.ts`): **24/24 passed** — 8 C11 scenarios × Chromium, Firefox, and WebKit:
...
- **Manual demo verification** (`http://localhost:5173/phase11.html`): all 5 debugger panels working; Vis toggle shows bounding boxes; preloader appears and dismisses; VCR record / rewind / step / play confirmed.

## Demo workbench

The interactive smoke workbench is at `http://localhost:5173/phase11.html` (via `npm run dev` / `vite --config vite.smoke.config.ts`).
