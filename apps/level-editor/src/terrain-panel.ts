...
export function terrainPanel(
  ...
  return `<div class="terrain-panel ${set?.kind === 'edge' ? 'terrain-edge' : ''}" style="--terrain-color:${set?.color ?? '#72a854'}">
    <div class="terrain-heading"><strong>Terrain sets</strong><span><button class="button ghost compact" type="button" data-terrain-add ${!asset || sets.length >= 64 ? 'disabled' : ''}>+ Corner set</button><button class="button ghost compact" type="button" data-terrain-edge-add ${!asset || sets.length >= 64 ? 'disabled' : ''}>+ Edge set</button></span></div><div class="terrain-interchange"><button class="button ghost compact" type="button" data-terrain-tiled-import ${!asset ? 'disabled' : ''}>Import Tiled JSON</button><button class="button ghost compact" type="button" data-terrain-tiled-export ${!asset || !sets.length ? 'disabled' : ''}>Export Tiled JSON</button></div>
    ...
    ${
      set
        ? `<label class="tileset-label">Terrain to paint<select aria-label="Terrain to paint" data-terrain-brush>${types.map((type, index) => `<option value="${index + 1}" ${(snapshot.terrain?.terrainIndex ?? 1) === index + 1 ? 'selected' : ''}>${html(type.name)}</option>`).join('')}</select></label><div class="terrain-tools" role="toolbar" aria-label="Terrain tools"><button type="button" data-tool="terrain" aria-pressed="${snapshot.tool === 'terrain'}"><span class="terrain-swatch" style="background:${paintedType?.color ?? set.color}"></span>Paint ${html(paintedType?.name ?? set.name)}</button><button type="button" data-tool="terrain-erase" aria-pressed="${snapshot.tool === 'terrain-erase'}">Erase terrain</button></div>${set.kind === 'edge' ? `<fieldset class="edge-brush-options"><legend>Road brush</legend><div class="field-pair"><label>Width<select aria-label="Road width" data-terrain-edge-width>${[1, 3, 5].map((width) => `<option value="${width}" ${(snapshot.terrain?.edgeWidth ?? 1) === width ? 'selected' : ''}>${width} cell${width === 1 ? '' : 's'}</option>`).join('')}</select></label><label>Stroke<select aria-label="Road stroke" data-terrain-edge-straight><option value="free" ${snapshot.terrain?.edgeStraight ? '' : 'selected'}>Freehand</option><option value="straight" ${snapshot.terrain?.edgeStraight ? 'selected' : ''}>Straight</option></select></label></div><label>Ends<select aria-label="Road endpoints" data-terrain-edge-ends><option value="caps" ${(snapshot.terrain?.edgeEnds ?? 'caps') === 'caps' ? 'selected' : ''}>Capped</option><option value="junctions" ${snapshot.terrain?.edgeEnds === 'junctions' ? 'selected' : ''}>Junctions</option></select></label><label class="edge-option-check"><input type="checkbox" data-terrain-edge-closed ${snapshot.terrain?.edgeClosed ? 'checked' : ''}/> Close loop on release</label><p class="field-help">Junction ends add four-way connections. Closed strokes connect the last cell back to the first.</p></fieldset>` : ''}
