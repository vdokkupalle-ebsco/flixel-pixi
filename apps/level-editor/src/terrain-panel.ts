  ...
  return `<div class="terrain-panel" style="--terrain-color:${set?.color ?? '#72a854'}">
    ...
    ${
      ...
      <details class="terrain-rules" ${editing ? 'open' : ''}><summary>Terrain rules <span>${set.rules.length}/${terrainPatternCount(set) - 1} patterns</span></summary>
        ...
        <p class="field-help">Dashed patterns need a tile. Empty always clears a cell. Corners can join any terrain types in this set. Assign tile replaces the primary tile. Add variant keeps it and adds another choice. A source tile can belong to only one pattern.</p>
    ...
    <button class="button ghost full terrain-sample" type="button" data-terrain-sample>Add sample terrain</button><button class="button ghost full terrain-sample" type="button" data-terrain-multi-sample>Add grass and dirt sample</button>
  </div>`;
}
