...
export interface PhaseSixState {
...
export interface PhaseSixMetrics {
...
export interface PhaseSixApplication {
  readonly metrics: PhaseSixMetrics;
...
function makeTileset(): FlxGraphic {
  ...
  return FlxGraphic.fromPixels(pixels, 'phase6-tiles');
}
...
export async function createPhaseSixApplication(
  host: HTMLElement,
): Promise<PhaseSixApplication> {
  ...
  const seek = (seconds: number): PhaseSixState => {
