...
export class PlatformServicesState extends FlxState {
  save = new FlxSave();
  coins = 0;
  coinText!: FlxText;
  statusText!: FlxText;
  ...
  override create(): void {
    ...
    this.statusText = new FlxText(
      10,
      30,
      300,
      'Platform services: Audio & Save Active',
    );
...
export interface PlatformServicesApplication {
...
export async function createPlatformServicesApplication(
  host: HTMLElement,
): Promise<PlatformServicesApplication> {
  ...
  return {
    ...
    get metrics(): PlatformServicesMetrics {
      const state = game.state as PlatformServicesState | null;
      return {
    ...
    async verifyIndexedDb() {
      const databaseName = `flixel-pixi-platform-services-${crypto.randomUUID()}`;
