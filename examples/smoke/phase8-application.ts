...
export interface PhaseEightMetrics {
...
export interface PhaseEightState {
...
export interface PhaseEightApplication {
  readonly metrics: PhaseEightMetrics;
...
export async function createPhaseEightApplication(
  host: HTMLElement,
): Promise<PhaseEightApplication> {
  ...
  app.canvas.setAttribute('aria-label', 'Phase 8 deterministic effects lab');
  ...
  const readState = (): PhaseEightState => ({
  ...
  renderer.render();
  const metrics: PhaseEightMetrics = {
