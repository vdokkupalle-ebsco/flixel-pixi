...
export interface PhaseFourMetrics {
...
export interface PhaseFourApplication {
  readonly metrics: PhaseFourMetrics;
  destroy(): void;
}
...
function createSheet(): FlxGraphic {
  ...
  return FlxGraphic.fromPixels(pixels, 'phase4-sheet');
}

export async function createPhaseFourApplication(
  host: HTMLElement,
): Promise<PhaseFourApplication> {
  ...
  const secondConfig = await assets.load('phase4-config');
  ...
  const title = new FlxText(24, 163, 432, 'Phase 4 · sprites + text')
  ...
  const metrics: PhaseFourMetrics = {
    animationCallbacks,
