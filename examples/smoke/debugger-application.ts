...
class PlayState extends FlxState {
  ...
  override create(): void {
    ...
    const title = new FlxText(20, 14, 600, 'DEBUGGER — DEBUGGER & PRELOADER');
...
export interface DebuggerApplication {
  game: FlxGame;
  debugger: FlxDebugger;
  destroy(): void;
}

export async function bootDebuggerDemo(
  host: HTMLElement,
): Promise<DebuggerApplication> {
  // ── Preloader ──────────────────────────────────────────────────────────────
  const preloader = new FlxPreloader({ title: 'Debugger Lab' });
  ...
  // Initial log message
  FlxG.log.add('Debugger Lab ready. Ball bouncing!', 0xff38bdf8);
  FlxG.log.add(
