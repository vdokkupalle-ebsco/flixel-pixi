...
export type CamerasCameraName = 'follow' | 'overview';

export interface CamerasResizeEvidence {
...
export interface CamerasAnimationState {
...
export interface CamerasMetrics {
  cameraCount: number;
...
export interface CamerasApplication {
  readonly metrics: CamerasMetrics;
  advanceAnimation(steps: number): CamerasAnimationState;
  animationState(): CamerasAnimationState;
  destroy(): void;
  pauseAnimation(): void;
  pointerToWorld(camera: CamerasCameraName, x: number, y: number): FlxPoint;
  resize(
    width: number,
    height: number,
    resolution: number,
...
const PATH_START_ANGLE = Math.asin(120 / 220);
...
export async function createCamerasApplication(
  host: HTMLElement,
): Promise<CamerasApplication> {
  ...
  if (followTarget === undefined || overviewTarget === undefined) {
    throw new Error('Camera targets were not created.');
  }
  ...
  const applyTargetPath = (): void => {
    ...
    shared.x = 400 + Math.sin(radians + PATH_START_ANGLE) * 220;
  ...
  const readAnimationState = (): CamerasAnimationState => ({
  ...
  return {
    metrics,
    advanceAnimation(steps): CamerasAnimationState {
    ...
    resize(width, height, resolution): CamerasResizeEvidence {
    ...
    seekAnimation(seconds): CamerasAnimationState {
